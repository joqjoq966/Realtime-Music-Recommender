...
def read_item(tag_name):
    ...
    if not all_model_results.get(tag_name):
    ...
    all_model_recommendations, all_model_spent = all_model_results.get(tag_name)
    all_recos = [songId2meta.get(sid) for sid in all_model_recommendations]

    svd_recommendations, svd_spent = svd_results.get(tag_name)
    svd_recos = [songId2meta.get(sid) for sid in svd_recommendations]
    
    return { 
        "total_length": 3,
        "cluster_model": {
            "spent": spent_time * 1000,
            "predictions": [
                {"songname": song_name, "singer": artist_name}
                for song_name, artist_name in recommendations]
        },
        "all_model": {
            "spent": all_model_spent,
            "predictions": [
                {"songname": song_name, "singer": artist_name}
                for song_name, artist_name in all_recos]
        },
        "svd_model": {
            "spent": svd_spent,
            "predictions": [
                {"songname": song_name, "singer": artist_name}
                for song_name, artist_name in svd_recos]
        }
    }
